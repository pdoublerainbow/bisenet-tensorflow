...
def _conv(inputs, num_filters, kernel_size, stride=1, dropout_rate=None,
          scope=None, outputs_collections=None, dilation_rate=1):
  with tf.variable_scope(scope, 'xx', [inputs]) as sc:
    net = slim.batch_norm(inputs)
    net = tf.nn.relu(net)
    net = slim.conv2d(net, num_filters, kernel_size, rate=dilation_rate)
...
@slim.add_arg_scope
def _conv_block(inputs, num_filters, data_format='NHWC', scope=None, outputs_collections=None, dilation_rate=1):
  with tf.variable_scope(scope, 'conv_blockx', [inputs]) as sc:
    net = inputs
    net = _conv(net, num_filters*4, 1, scope='x1')
    net = _conv(net, num_filters, 3, scope='x2', dilation_rate=dilation_rate)
    if data_format == 'NHWC':
...
def _dense_block(inputs, num_layers, num_filters, growth_rate,
                 grow_num_filters=True, scope=None, outputs_collections=None, dilation_rate=1):

  with tf.variable_scope(scope, 'dense_blockx', [inputs]) as sc:
    net = inputs
    for i in range(num_layers):
      branch = i + 1
      net = _conv_block(net, growth_rate, scope='conv_block'+str(branch), dilation_rate=dilation_rate)
...
def _transition_block(inputs, num_filters, compression=1.0,
                      scope=None, outputs_collections=None, stride=2):
...
def densenet(inputs,
  ...
  with tf.variable_scope(scope, 'densenetxxx', [inputs],
                         reuse=reuse) as sc:
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      dilation = [1, 1, 2, 4]
      transition_stride = [2, 2, 1, 1]
      # blocks
      for i in range(num_dense_blocks):
        # dense blocks
        net, num_filters = _dense_block(net, num_layers[i], num_filters,
                                        growth_rate,
                                        scope='dense_block' + str(i+1), dilation_rate=dilation[i])

        # Add transition_block
        net, num_filters = _transition_block(net, num_filters,
                                             compression=compression,
                                             scope='transition_block' + str(i+1), stride=transition_stride[i])
...
def densenet161(inputs, num_classes=None, data_format='NHWC', is_training=True, reuse=None, scope='densenet161'):
  return densenet(inputs,
                  num_classes=num_classes, 
                  reduction=0.5,
                  growth_rate=48,
...
def densenet169(inputs, num_classes=None, data_format='NHWC', is_training=True, reuse=None, scope='densenet169'):
  return densenet(inputs,
                  num_classes=num_classes, 
                  reduction=0.5,
                  growth_rate=32,
